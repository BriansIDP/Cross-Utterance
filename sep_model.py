...
class RNNModel(nn.Module):
    ...
    def forward(self, input, hidden, separate=0, eosidx = 0, target=None, outputflag = 0, sep=False, length=None):
        if sep:
            padded_emb = self.drop(self.encoder(input.data))
            emb = nn.utils.rnn.pack_padded_sequence(padded_emb, length)
        else:
        ...
        if self.loss_type == 'nce':
            ...
            if sep:
                decoded = self.decoder(output.data.view(output.size(0)*output.size(1), output.size(2)))
            else:
